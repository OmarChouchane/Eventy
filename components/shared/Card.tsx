    ...
    <CardContainer className="inter-var flex-shrink-0 w-[320px] sm:w-[360px] lg:w-[400px]">
      <CardBody className="bg-black relative group/card dark:hover:shadow-2xl dark:hover:shadow-emerald-500/[0.1] dark:bg-black dark:border-white/[0.2] border-black/[0.1] w-full min-h-[380px] md:min-h-[438px] rounded-xl border flex flex-col overflow-visible shadow-md transition-all hover:shadow-lg">
        ...
        <HoverSpotlight
          ...
          spotlightClassName="opacity-0 group-hover/card:opacity-70"
        >
          <CardItem translateZ="60" className="flex-grow overflow-visible z-10">
            <Link
              href={`/events/${event._id}`}
              tabIndex={-1}
              className="relative block h-[200px] w-full rounded-xl group"
            >
              <Image
                src={event.imageUrl}
                alt={event.title}
                width={400}
                height={200}
                className="h-full w-full object-cover transition-transform duration-500 ease-out origin-center group-hover/card:scale-110 group-hover/card:-translate-y-3 group-hover/card:-translate-x-3 z-0 will-change-transform"
              />
